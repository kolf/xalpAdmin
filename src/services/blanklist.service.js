...
class BlacklistService {
  ...
  updateBlockAllowUser = async (creds) => {
    try {
      const res = await api.post(`/api​/BlockAllowUser​/${creds.id}`, creds);
      return res.data;
    } catch (error) {
      return Promise.reject(error);
    }
  ...
  deleteBlockAllowUser = async (creds) => {
    try {
      const res = await api.delete(`/api/BlockAllowUser/${creds.id}`);
      return res.data;
    } catch (error) {
      return Promise.reject(error);
    }
  ...
  updateBlockBehavior = async (creds) => {
    try {
      const res = await api.put(`api/BlockBehavior/${creds.id}`);
      return res.data;
    } catch (error) {
      return Promise.reject(error);
    }
